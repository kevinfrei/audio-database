...
beforeAll(async () => {
  ...
  root = path.resolve('src/__tests__/persist-basic');
  persist = MakePersistence(root);
});

describe('Metadata testing', () => {
  test('Full/Partial Metadata tests', () => {
  ...
  test('Generic getMediaInfo tests', async () => {
...
describe('Metadata Store testing #1', () => {
  test('Basic Metadata Store routines', async () => {
...
describe('Metadata Store testing #2', () => {
  test('Yet another Metadata Store', async () => {
    const mds = await GetMetadataStore(persist, 'basic', root);
    const someFile = mds.get('./data/someFile.mp3');
